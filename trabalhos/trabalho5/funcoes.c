...
const int tamanho_banco_terminal[MAX_TERMINAL] = {181,31,83,23};

itens **separa_itens(char **entradas, int quantidade){
  /*
   * lendo -> salva o byte que e' para partir a leitura
   * r -> retorno da leitura
   * bytes -> bytes lido na leitura
   * item -> onde salvar o numero lido
   * flaot -> numero de entrada lido
   */
  ...
  for (int i = 0; i < quantidade; i++){
    ...
    while ((r = sscanf(&(entradas[i][lendo]), "%f %n", &lido, &bytes)) != EOF) {//tenta ler um flaot
      ...
      if(r == 1){//se acho um numero
        ...
        r = sscanf(&(entradas[i][lendo]), "%s %n", op, &bytes); // tenta ler um char
        if(r == 1){//acho um char

          if(strcmp(op, "ERRO") == 0){
            dados[i-1]->item4 = SENTINELA;
          }
          if(item == 1){
...
terminal **processa_entrada(itens **entradas, int qnt, int *auditoria){
  ...
  for(int i = 0; i<MAX_TERMINAL; i++){
    ...
    t[i]->e = (erros*)calloc(1, sizeof(erros));
  }

  for(int i = 0; i < qnt; i++){//analisza cada operacao entrada

    if(entradas[i]->item3 == SENTINELA){//se cehgar em operação de AUDITORIA

      *auditoria = i;
      break;
    }

    if(entradas[i]-> item4 == SENTINELA){//se achar erro
      continue;
    }

    if(strcmp(entradas[i]->o, "ERRO") == 0){//verifica se for erro
      processa_erro(entradas[i-1]->o, t[(int)entradas[i-1]->item1-1]->e);
    }
...
void passa_para_terminal(itens *operacao, terminal *t){
  t->operacoes[t->qnt_operacoes++] = operacao; //salva operacao
  ...
  if(t->bancos[valor_hash] == NULL){//se não ja tiverem acessado o banco
    ...
    if(!(strcmp(operacao->o, "T") == 0 && operacao->item4 == 0.0) && strcmp(operacao->o, "C") != 0){//para nao salvar transferencia entre o mesmo banco e consultas
...
int salva_nova_operacao_banco(itens *operacao, banco *b){
  //vejo qual operação é e salva quantidade de dinheiro movimentadas
  ...
  if(strcmp(operacao->o, "S") == 0){//se for saque
    ...
    if(operacao->item4 != 0){//transferencia entre bancos diferentes
...
void relatorio(terminal **t){
  ...
  e_f = (erros*)calloc(1, sizeof(erros));
...
void salva_erro_final(erros *e_f, erros *t){
  e_f->erro += t->erro;
  e_f->saque += t->saque;
  e_f->deposito += t->deposito;
  e_f->transferencia += t->transferencia;
  e_f->consulta += t->consulta;
...
void salva_final(banco_final *bf, banco *salvar){
  for(int i =0 ; i < bf->qnt; i++){
    if(bf->i_bancos[i].id == salvar->id){ //ve se o banco ja esta nos dados finais, se sim salva as infos deles
  ...
  //cria novo espaço para um banco novo
...
void imprime_auditoria(int apartir_de, int total, itens **entradas, terminal **terminais){
  ...
  for(int i = apartir_de; i < total-1; i++){
    ...
    while((j < (int)entradas[i]->item2) && (k < terminais[(int)entradas[i]->item1-1]->qnt_operacoes)){//enquanto nao imprimei a quantidade pedida ou chego no final dos itens
